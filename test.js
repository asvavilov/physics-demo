(function() {
  ...
  function init() {
    ...
    img.addEventListener("load", function() {
      ...
      new Body(physics, { color: "red", type: "static", x: 0, y:25, height: 0.5, width: 120, friction: 0.1 });

	  new Body(physics, { image: img, x: 5, y: 8, width: 3, height: 3, density: 5 });

		var body_a = new Body(physics, { image: img, x: 13, y: 8 });
		var body_b = new Body(physics, { color: "blue", x: 17, y: 0, angle: 45 });
		def = new Box2D.Dynamics.Joints.b2DistanceJointDef();
		def.Initialize(body_a.body,
					   body_b.body,
					   body_a.body.GetWorldCenter(),
					   body_b.body.GetWorldCenter());
		def.dampingRatio = 0.05;
		def.frequencyHz = 1;
		physics.world.CreateJoint(def);
		
      new Body(physics, { color: "gray", image: img, width: 5, height: 5, shape: "circle", radius: 4, x: 5, y: 20 });

      new Body(physics, { color: "pink", shape: "polygon", friction: 0.1,
                          points: [ { x: 0, y: 0 }, { x: 0, y: 4 },{ x: -10, y: 0 }   ],
                          x: 20, y: 5 });

      physics.click(function(body) {
var ix = Math.random()*1000-500;
var impulse =
{ x: ix, y: -500 }
;
var point = body.GetWorldCenter();
        body.ApplyImpulse(impulse, point);
		body.ApplyTorque((ix > 0 ? 1 : -1) * 100);
      });
